...
//命令分发
function dispatchCommand(src_path,worker,cmd){
	if(global.master.initOk){			//初始化成功
		if(!global.master.appCmdOk && msg.type == "app"){		
			return ;					//请首先启动命令处理
		}
		var pre = src_path + "\\commands";
		var mid = "_" + worker;
...
function initCommand(self){
	...
	self.cluster.on('message', (worker, message, handle) => {
		...
		if(["console_input","cmd_source","print"].indexOf(worker.var_name) > -1){
			dispatchCommand(self.src_path,worker.var_name,cmd);
		}else{
...
function init(cluster){
	...
	console_output.init_info();									//初始化输出	
